  ...
  getTranscript: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    ...
    const userSpeakers = await db
      ...
      .then((users) =>
        users.map((user) => ({
          ...user,
          image: user.image ?? generateAvatarUrl({ variant: "initials", seed: user.name }),
        }))
      );
    const agentSpeakers = await db
      ...
      .then((agents) =>
        agents.map((agent) => ({
          ...agent,
          image: generateAvatarUrl({ variant: "botttsNeutral", seed: agent.name }),
        }))
      );
  ...
  getMany: protectedProcedure
    .input(
      z.object({
        ...
        status: z
          .enum([MeetingStatus.Upcoming, MeetingStatus.Active, MeetingStatus.Cancelled, MeetingStatus.Completed, MeetingStatus.Processing])
          .nullish(),
      })
    )
